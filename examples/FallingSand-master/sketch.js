...
const PLACEABLE_TYPES = {
  ...
  "Sand Source": function (x, y) {
    return new ParticleSource(x, y, world, SandParticle);
  },
  "Water Source": function (x, y) {
    return new ParticleSource(x, y, world, WaterParticle);
  },
  "Steam Source": function (x, y) {
    return new ParticleSource(x, y, world, SteamParticle);
  },
  "Flame Source": function (x, y) {
    return new ParticleSource(x, y, world, FlameParticle);
  },
  "Gasoline Source": function (x, y) {
    return new ParticleSource(x, y, world, GasolineParticle);
  },
  "Hydrogen Source": function (x, y) {
    return new ParticleSource(x, y, world, HydrogenParticle);
  },
  "Gunpowder Source": function (x, y) {
    return new ParticleSource(x, y, world, GunpowderParticle);
  },
...
function setup() {
  ...
  p5canvas = createCanvas(
    pixelsPerParticle * world.gridWidth,
    pixelsPerParticle * world.gridHeight
  );
  // pixelDensity(1);
  rc = new RadianceCascades(p5canvas);
  ...
  brushSizeSlider = createSlider(
    1,
    min(16, min(world.gridWidth, world.gridHeight)),
    2,
    1
  );
  ...
  resizeButton = createButton(
    "Reset & Resize World<br>(may affect performance)"
  );
...
updateCanvasSize = function () {
  resizeCanvas(
    world.gridWidth * pixelsPerParticle,
    ...
    if (
      ...
      for (i = imin; i < imin + brushSize; i++) {
        ...
        for (j = imin; j < imin + brushSize; j++) {
          let iy = y + j;
          if (
            ix <= world.gridWidth - 2 &&
            ix >= 1 &&
            iy <= world.gridHeight - 2 &&
            iy >= 1
          ) {
