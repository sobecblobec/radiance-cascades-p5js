class Particle extends Placeable {
    ...
    if (b) {
      ...
      if (this instanceof FluidParticle) {
        ...
        this.color = adjustHSBofString(
          ...
          1,
          1,
          map(this.fuel, 0, this.originalFuel, 0.2, 1)
        );
      }
    }
    this._burning = b;
  ...
  burningFlickerColor() {
    return adjustHSBofString(
      this.constructor.BURNING_COLOR,
      random(0.95, 1.05),
      random(0.95, 1.05),
      random(0.95, 1.05)
    );
...
class MoveableParticle extends Particle {
  ...
  tryGridPosition(x, y, trySwap = true) {
    ...
    else if (trySwap && p instanceof MoveableParticle && !p.hasBeenDisplaced) {
...
class SteamParticle extends FluidParticle {
  ...
  constructor(x, y, world) {
    super(x, y, world);
    this.weight = 0.5;
    this.initialConensationCountdown = round(
      this.constructor.BASE_CONDENSATION_COUNTDOWN * random(0.7, 1.3)
    );
